...
func NewDestinationAddress() (c DestinationAddress) {
	c.destFlag = constants.DFLT_DEST_FLAG
	return
}
...
func (c *DestinationAddress) Unmarshal(b *ByteBuffer) (err error) {
	if c.destFlag, err = b.ReadByte(); err == nil {
		...
		case constants.SM_DEST_SME_ADDRESS:
			err = c.address.Unmarshal(b)

		case constants.SM_DEST_DL_NAME:
			err = c.dl.Unmarshal(b)

		default:
			err = fmt.Errorf("unrecognized dest_flag %d", c.destFlag)
...
func (c *DestinationAddress) Marshal(b *ByteBuffer) {
	switch c.destFlag {
	case constants.SM_DEST_DL_NAME:
		_ = b.WriteByte(constants.SM_DEST_DL_NAME)
		c.dl.Marshal(b)

	default:
		_ = b.WriteByte(constants.SM_DEST_SME_ADDRESS)
		c.address.Marshal(b)
	}
}
...
func (c *DestinationAddress) SetAddress(addr Address) {
	c.destFlag = constants.SM_DEST_SME_ADDRESS
	c.address = addr
}
...
func (c *DestinationAddress) IsDistributionList() bool {
	return c.destFlag == byte(constants.SM_DEST_DL_NAME)
}
