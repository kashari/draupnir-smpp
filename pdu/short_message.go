...
func (c *ShortMessage) SetMessageWithEncoding(message string, enc data.Encoding) (err error) {
	if c.messageData, err = enc.Encode(message); err == nil {
		if len(c.messageData) > constants.SM_MSG_LEN {
		...
		if c.enc == data.GSM7BITPACKED { // to prevent unwanted "@"
			...
			tLen := len(runeSlice)
			escCharsLen := len(encoding.GetEscapeChars(runeSlice))
...
func (c *ShortMessage) split() (multiSM []*ShortMessage, err error) {
	...
	if !ok || !splitter.ShouldSplit(c.message, constants.SM_GSM_MSG_LEN) {
	...
	segments, err := splitter.EncodeSplit(c.message, constants.SM_GSM_MSG_LEN-6)
	if err != nil {
		return nil, err
	}
