...
func NewBindResp(req BindRequest) (c *BindResp) {
	...
	case Transceiver:
		c.CommandID = constants.BIND_TRANSCEIVER_RESP

	case Receiver:
		c.CommandID = constants.BIND_RECEIVER_RESP

	case Transmitter:
		c.CommandID = constants.BIND_TRANSMITTER_RESP
	}
...
func (c *BindResp) Unmarshal(b *ByteBuffer) error {
	return c.base.unmarshal(b, func(w *ByteBuffer) (err error) {
		if c.CommandID == constants.BIND_TRANSCEIVER_RESP || c.CommandStatus == constants.ESME_ROK {
