...
import (
	enc "github.com/kashari/draupnir/encoding"
...
func (c *gsm7bit) Encode(str string) ([]byte, error) {
	return encode(str, enc.GSM7(c.packed).NewEncoder())
}

func (c *gsm7bit) Decode(data []byte) (string, error) {
	return decode(data, enc.GSM7(c.packed).NewDecoder())
}
...
func (c *gsm7bitPacked) Encode(str string) ([]byte, error) {
	return encode(str, enc.GSM7(true).NewEncoder())
}

func (c *gsm7bitPacked) Decode(data []byte) (string, error) {
	return decode(data, enc.GSM7(true).NewDecoder())
}
...
func (c *gsm7bitPacked) ShouldSplit(text string, octetLimit uint) (shouldSplit bool) {
	runeSlice := []rune(text)
	tLen := len(runeSlice)
	escCharsLen := len(enc.GetEscapeChars(runeSlice))
...
func determineTo(from int, to int, lim int, runeSlice []rune) int {
	nSeptet := 0
	for nSeptet < lim {
		if enc.IsEscapeChar(runeSlice[from]) { // esc chars counted as 2 septes
			nSeptet += 2
		} else {
			nSeptet++
		}
	...
	if enc.IsEscapeChar(runeSlice[to-1]) { // 9.2.3.24.1 Concatenated Short Messages  "A character represented by an escape-sequence shall not be split in the middle."
		if nSeptet > lim {
			to--
		}
	}
...
func (*binary8bit1) Encode(_ string) ([]byte, error) {
	return []byte{}, enc.ErrNotImplEncode
}

func (*binary8bit1) Decode(_ []byte) (string, error) {
	return "", enc.ErrNotImplDecode
}
